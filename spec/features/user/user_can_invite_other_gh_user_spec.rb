...
describe 'A registered user', :vcr do
  ...
  it 'lets user send email to invite github user', :vcr do
    ...
    allow_any_instance_of(ApplicationController).to receive(:current_user).and_return(user)
    stub_request(:get, "https://api.github.com/users/nicklindeberg").with(headers: {"Authorization" => 'GITHUB_TOKEN_2' }).to_return(body: File.read("./spec/fixtures/invite.json"))
